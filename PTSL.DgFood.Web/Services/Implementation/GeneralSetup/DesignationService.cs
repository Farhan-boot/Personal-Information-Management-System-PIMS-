...
namespace PTSL.DgFood.Web.Services.Implementation.GeneralSetup
{
    public class DesignationService : IDesignationService
    {
        ...
        public (ExecutionState executionState, List<DesignationVM> entity, string message) GetByRankAndDesignationClassId(long? rankId, long? designationClassId)
        {
            (ExecutionState executionState, List<DesignationVM> entity, string message) returnResponse;
            try
            {
                DesignationVM model = new DesignationVM();
                var respJson = JsonConvert.SerializeObject(model);
                var URL = string.Concat(URLHelper.ApiBaseURL, $"{URLHelper.DesignationGetByRankAndDesignationClass}?rankId={rankId ?? 0}&designationClassId={designationClassId ?? 0}");
                var json = new HttpHelper().Get(URL);
                WebApiResponse<List<DesignationVM>> responseJson = JsonConvert.DeserializeObject<WebApiResponse<List<DesignationVM>>>(json);
                returnResponse.executionState = responseJson.ExecutionState;
                returnResponse.entity = responseJson.Data;
