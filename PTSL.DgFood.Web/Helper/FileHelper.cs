...
namespace PTSL.GENERIC.Web.Core.Helper
{
	...
	{
		public const string Uploads = "Content";
		...
		public bool IsValidDocument(HttpPostedFileBase file)
		{
			if (file is null) return false;
		...
		public (bool, string, string) SaveFile(HttpPostedFileBase file, FileType fileType, string directoryName, out string errorMessage)
		{
			if (file is null)
			{
			...
			var uploadDirectory = Path.GetFullPath(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, Uploads));
